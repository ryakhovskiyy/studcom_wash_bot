...
class SheetManager:
    ...
    def get_user(self, telegram_id: int):
        try:
            ...
            return self.users_sheet.row_values(cell.row) if cell else None
    ...
    def update_user_field(self, telegram_id: int, field_name: str, value: str):
        try:
            ...
            print(f"Ошибка при обновлении поля '{field_name}' для {telegram_id}: {e}")
    ...
    def get_unique_column_values(self, sheet_name: str, column_name: str) -> list:
        ...
        for record in all_records:
            try:
                ...
                slot_datetime = datetime.strptime(slot_datetime_str, '%d.%m.%Y %H:%M')
    ...
    def get_available_slots(self, filters: dict) -> list:
        ...
        for i, slot in enumerate(all_slots):
            ...
            if selected_times:
                ...
                time_match = False
                if 'Утро' in selected_times and (8 <= time_h < 12): time_match = True
                if not time_match and 'День' in selected_times and (12 <= time_h < 18): time_match = True
                if not time_match and 'Вечер' in selected_times and (18 <= time_h < 22): time_match = True
                if not time_match: continue

            slot['row_index'] = i + 2
            available.append(slot)

        available.sort(key=lambda x: (datetime.strptime(x['slot_date'], '%d.%m.%Y'), x['start_time']))
        return available
    ...
    def get_user_bookings(self, telegram_id: int, upcoming_only: bool = True) -> list:
        ...
        for i, booking in enumerate(all_bookings):
            if str(booking.get('telegram_id')) == str(telegram_id):
                ...
                if upcoming_only:
                    try:
                        # Сравниваем не просто даты, а полное время
                        slot_dt_str = f"{booking['slot_date']} {booking['start_time']}"
                        slot_dt_naive = datetime.strptime(slot_dt_str, '%d.%m.%Y %H:%M')
                        slot_dt_aware = moscow_tz.localize(slot_dt_naive)

                        if slot_dt_aware > now_aware and booking.get('status') == 'Booked':
                    ...
                    booking['archive_row_index'] = i + 2
                    user_bookings.append(booking)
        user_bookings.sort(key=lambda x: (datetime.strptime(x['slot_date'], '%d.%m.%Y'), x['start_time']), reverse=True)
        return user_bookings

    def book_slot(self, slot_data: dict, user_info: dict) -> dict | None:
        """
        Бронирует слот, добавляя ФИО и ответственного в архив.
        Возвращает словарь с данными о брони.
        """
        try:
            target_row_index = slot_data['row_index']

            actual_slot_data = dict(zip(self._schedule_headers, self.schedule_sheet.row_values(target_row_index)))
            if not actual_slot_data or not actual_slot_data.get('slot_date'):
                return None
            ...
            archive_row = {header: actual_slot_data.get(header, '') for header in self._archive_headers}
            archive_row.update({
                'telegram_id': user_info['id'],
                'username': user_info['username'],
                'full_name': full_name,
                'booking_timestamp': datetime.now().strftime('%d.%m.%Y %H:%M'),
                'status': 'Booked'
            })
        ...
        except (gspread.exceptions.APIError, gspread.exceptions.CellNotFound, IndexError):
            return None

    def cancel_booking(self, archive_row_index: int) -> bool:
        """
        Отменяет бронь: возвращает "чистый" слот (с ответственным) в schedule
        и меняет статус в archive.
        """
        try:
            ...
            # ИСПРАВЛЕНИЕ: Готовим чистую строку, сохраняя ответственного
            slot_to_return = [
                booking_to_cancel.get(header, '') for header in self._schedule_headers
            ]
            ...
            print(f"Ошибка при отмене брони (строка {archive_row_index}): {e}")
            return False
